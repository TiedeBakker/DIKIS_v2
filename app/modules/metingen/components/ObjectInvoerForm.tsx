...
// 1. Pas dit aan naar wat de database daadwerkelijk teruggeeft:
interface Parameter { 
  id: string; 
  naam: string; 
  toelichting: string | null;
  parametertypeId: string;
  eenheidId: string | null; // <-- Drizzle spuugt eenheidId uit, niet de tekst 'eenheid'
...
export default function ObjectInvoerForm({ objectenView, sets, parameters }: Props) {
    ...
    // Bepaal welke velden getoond moeten worden (inclusief eenheid!)
    useEffect(() => {
        setFormWaarden({});
            ...
            getParametersVoorSet(geselecteerdeSetId).then((regels) => {
                const velden = regels.map(r => {
                    ...
                    return {
                        ...
                        // We combineren hier het label/naam met de eenheid als deze bestaat
                        label: r.label || stamParam?.naam || 'Onbekend',
                        eenheid: stamParam?.eenheidId || null, // <-- NIEUW: eenheid meenemen
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault(); // Voorkomt de standaard browser submit
        ...
        // 2. NIEUW: Verificatie-melding voor de gebruiker
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
        if (e.key === 'Enter') {
            ...
            // Zoek het volgende invoerveld op basis van de index
            const volgendeVeld = document.getElementById(`param-input-${index + 1}`);
            if (volgendeVeld) {
                (volgendeVeld as HTMLInputElement).focus();
            } else {
                // Als er geen volgend veld is, zijn we bij de laatste. 
                // Dan mag de focus naar de opslaan-knop springen.
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto bg-white p-4 md:p-6 rounded-lg border border-slate-200 shadow-sm space-y-5">
            ...
            {actieveVelden.length > 0 && (
                <div className="bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-200 space-y-4">
                            ...
                            return (
                                <div key={veld.parameterId} className="flex flex-col space-y-1 bg-white p-2.5 rounded border border-slate-100 shadow-xs">
                                    ...
                                    <input
                                        ...
                                        placeholder={veld.placeholder || (veld.eenheid ? `Bijv. in ${veld.eenheid}` : '')}
                    ...
                    <button
                        id="btn-submit-meting" // <-- ID toegevoegd voor de focus-stap
                        type="submit"
                        disabled={isPending}
